...
queen %>%
  group_by(album_name) %>%
  ggplot(aes(x = album_name, y = (duration_ms/60000), fill = album_name)) +
  geom_point()
...
```{r}
ggplot(queenkey, aes(x = key_name, fill = album_name, y = n, 
...
```
```{r}
ggplot(queen, aes(x=valence, fill=album_name,
